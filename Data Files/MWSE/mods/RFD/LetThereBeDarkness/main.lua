...
local useroverridescells = require("RFD.LetThereBeDarkness.useroverridescells")
local useroverrideslights = require("RFD.LetThereBeDarkness.useroverrideslights")
...
local function lightsOff(cell)
	...
	oT = useroverridescells.overrideTableUser
	if (config.cellOverrideUser) and (oT[cellName]) then
		cell.ambientColor.r = (oT[cellName].ambientColor.r)
		cell.ambientColor.g = (oT[cellName].ambientColor.g)
		cell.ambientColor.b = (oT[cellName].ambientColor.b)
		cell.fogColor.r = (oT[cellName].fogColor.r)
...
--data exporting
local function compareCells()
    --
end

--data exporting
local function printCellsAllInterior()
    local temp = {}
    ...
    for i=1, #cells do
		...
		if (cells[i].isInterior == true) then
			local printthis = cells[i].id:lower()
			mwse.log("		[\"%s\"] = {", printthis)
			mwse.log("			ambientColor = tes3vector3.new(%.0f,%.0f,%.0f),", cells[i].ambientColor.r, cells[i].ambientColor.g, cells[i].ambientColor.b)
			mwse.log("			fogColor = tes3vector3.new(%.0f,%.0f,%.0f),", cells[i].fogColor.r, cells[i].fogColor.g, cells[i].fogColor.b)
			mwse.log("			sunColor = tes3vector3.new(%.0f,%.0f,%.0f),", cells[i].sunColor.r, cells[i].sunColor.g, cells[i].sunColor.b)
			mwse.log("		},")
		end
	end
end

local function openDataExporting()
...
local function openLiveLightEditing()
	...
	if (tes3.player == nil) then
		tes3.messageBox("[Let There Be Darkness]\nGame must be loaded to edit lighting values.\n\nIn this mode, lighting data exporters are accessible.\n\nMODDERS/ADVANCED USERS ONLY.")
...
local function editLights()
		...
		--user overrides
			if (config.lightOverrideUser == true) then
				local oT = useroverrideslights.overrideLightUser
				if (oT[lightName]) then
					if (oT[lightName].color) then
						light.color[1] = (oT[lightName].color.r)
						light.color[2] = (oT[lightName].color.g)
						light.color[3] = (oT[lightName].color.b)
					end
					if (oT[lightName].radius) then
						light.radius = (oT[lightName].radius)
					end
					if (oT[lightName].time) then
						light.time = (oT[lightName].time)
					end
					if (oT[lightName].value) then
						light.value = (oT[lightName].value)
					end
					if (oT[lightName].weight) then
						light.weight = (oT[lightName].weight)
					end
				end
			end
